...
async fn test_subprocessor_analyzer_creation() {
    ...
    assert!(
        !analyzer.is_valid_vendor_domain("example.com"),
        "Should filter placeholder domains"
    );
    assert!(
        analyzer.is_valid_vendor_domain("google.com"),
        "Should accept real domains"
    );
}
...
async fn test_end_to_end_analysis_with_invalid_domain() {
    ...
    let result = extract_vendor_domains_from_subprocessors(
    ...
    match result {
        Ok(vendors) => {
            ...
            assert!(
                vendors.is_empty() || vendors.len() < 5,
                "Should not find many vendors for fake domain"
            );
        }
        Err(_) => {
...
async fn test_deduplication_logic() {
    ...
    let vendors = analyzer
        .extract_from_lists(&document, html_with_duplicates, "https://example.com")
        .unwrap();
    ...
    assert!(
        deduplicated_vendors.len() >= 2,
        "Should have at least 2 unique domains after deduplication"
    );
    assert!(
        deduplicated_vendors.len() <= 2,
        "Should have at most 2 unique domains after deduplication"
    );
}
...
async fn test_content_type_handling() {
    ...
    let test_cases = vec![
        (
            "Analytics provided by Google Inc. at analytics.google.com",
            true,
        ),
        ("Email services by Mailgun LLC via mailgun.org", true),
        (
            "Cloud hosting through Amazon Web Services at aws.amazon.com",
            true,
        ),
...
async fn test_url_generation_patterns() {
    ...
    for domain in test_domains {
        ...
        assert!(
            urls.len() >= 25,
            "Should generate at least 25 URLs for domain: {}",
            domain
        );
        ...
        let with_www = urls
            .iter()
            .any(|url| url.contains(&format!("https://www.{}/", domain)));
        let without_www = urls
            .iter()
            .any(|url| url.contains(&format!("https://{}/", domain)));

        assert!(
            with_www || without_www,
            "Should generate appropriate URL variants for: {}",
            domain
        );
