...
fn measure_regex_overhead_detailed() {
    ...
    for _ in 0..10 {
        let _ =
            dns::extract_vendor_domains_with_source_and_logger(&complex_records, None, "test.com");
    }
    ...
    for _ in 0..1000 {
        let _ =
            dns::extract_vendor_domains_with_source_and_logger(&single_record, None, "test.com");
    }
    ...
    println!(
        "  Estimated regex compilation overhead: ~{:?} per call\n",
    ...
    println!(
        "    Time saved: ~{} seconds per 100k records",
        (duration_100.as_secs() * 100).saturating_sub(10)
    );
}
...
fn measure_domain_validation_overhead() {
    ...
    let records: Vec<String> = test_domains
        .iter()
    ...
    println!(
        "{} domains × 100 iterations ({} validations):",
        records.len(),
        records.len() * 100
    );
    ...
    println!(
        "  Per validation: {:?}",
        duration / (records.len() as u32 * 100)
    );
    println!(
        "  Validations/sec: ~{}\n",
...
fn measure_spf_macro_stripping_overhead() {
    ...
    println!(
        "{} macro records × 100 iterations ({} total):",
        records.len(),
        records.len() * 100
    );
    println!("  Total time: {:?}", duration);
...
fn estimate_total_optimization_gain() {
    ...
    let start = Instant::now();
    let domains =
        dns::extract_vendor_domains_with_source_and_logger(&mixed_records, None, "test.com");
    let duration = start.elapsed();
    ...
    println!(
        "  - Analyzing 10,000 domains: {}s → <10s",
        duration.as_secs() * 10
    );
    println!(
        "  - Analyzing 100,000 domains: {}s → <100s",
