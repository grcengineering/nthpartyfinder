...
// The vendor JSON files under `config/vendors/`, embedded at build time so the binary works with
// no `config/` directory on disk (Homebrew / crates.io / Docker / raw tarball). Generated by
// `build.rs` into `EMBEDDED_VENDOR_FILES: &[(&str, &str)]` = (file_name, file_contents). A
// user-provided `config/vendors/` still takes precedence at runtime (see `init`).
include!(concat!(env!("OUT_DIR"), "/embedded_vendors.rs"));
...
impl VendorRegistry {
    ...
    pub fn load_from_directory(config_dir: &Path) -> Result<Self> {
        ...
        for result in parsed_configs {
            match result {
                Ok(config) => {
                    registry.merge_vendor_config(config);
                }
    ...
    /// Build a registry from the vendor JSON files embedded at build time
    /// ([`EMBEDDED_VENDOR_FILES`]). Used when no on-disk `config/vendors/` directory is found —
    /// e.g. a Homebrew / crates.io / Docker / raw-tarball install — so the binary is fully
    /// self-contained instead of silently degrading to an empty registry. Parses the same JSON
    /// through the same [`merge_vendor_config`] path as [`load_from_directory`], so the two
    /// produce identical maps for identical inputs.
    pub fn load_from_embedded() -> Self {
        let mut registry = Self::new();
        for (name, content) in EMBEDDED_VENDOR_FILES {
            match serde_json::from_str::<VendorConfig>(content) {
                Ok(config) => {
                    registry.merge_vendor_config(config);
                }
                Err(e) => warn!("Failed to parse embedded vendor {}: {}", name, e),
            }
        }
        info!("Loaded {} vendors (embedded)", registry.vendors.len());
        registry
    }

    /// Insert one parsed vendor config into the registry's lookup maps. Shared by the filesystem
    /// (`load_from_directory` / `load_vendor_file`) and embedded (`load_from_embedded`) load paths
    /// so all three build byte-identical maps for identical input.
    fn merge_vendor_config(&mut self, config: VendorConfig) -> Arc<VendorConfig> {
    ...
    fn load_vendor_file(&mut self, path: &Path) -> Result<Arc<VendorConfig>> {
        ...
        let config: VendorConfig = serde_json::from_str(&content)
            .with_context(|| format!("Failed to parse: {:?}", path))?;
        Ok(self.merge_vendor_config(config))
    }
...
pub fn init() -> Result<()> {
    ...
    let registry = if let Some(ref dir) = config_dir {
        ...
        debug!("No config/vendors directory found; using embedded vendor data");
        VendorRegistry::load_from_embedded()
    };
...
mod tests {
    ...
    // ---- embedded vendor registry (self-contained binary) ----

    #[test]
    fn embedded_vendor_files_are_present() {
        // build.rs embeds every config/vendors/*.json (minus the schema). If the embed is empty the
        // binary would silently fall back to WHOIS/domain inference on every non-dev install.
        assert!(
            EMBEDDED_VENDOR_FILES.len() > 100,
            "expected the vendor directory to embed, got {} files",
            EMBEDDED_VENDOR_FILES.len()
        );
        assert!(
            EMBEDDED_VENDOR_FILES
                .iter()
                .all(|(name, _)| *name != "_schema.json"),
            "the JSON schema file must not be embedded as a vendor"
        );
    }

    #[test]
    fn load_from_embedded_parses_every_file_into_a_populated_registry() {
        let reg = VendorRegistry::load_from_embedded();
        // A populated registry, not the old empty fallback.
        assert!(
            reg.vendor_count() > 100,
            "embedded registry should be populated, got {}",
            reg.vendor_count()
        );
        // Every embedded file parsed into a vendor (no silent parse drops turning the embed into a
        // partial registry). One vendor id per file, so counts match exactly.
        assert_eq!(
            reg.vendor_count(),
            EMBEDDED_VENDOR_FILES.len(),
            "a vendor JSON failed to parse from the embedded set"
        );
        assert!(reg.domain_count() >= reg.vendor_count());
    }
