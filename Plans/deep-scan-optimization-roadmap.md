...
>
> **Implementation status (2026-08-21 update):** The DNS flapping investigation that gated P2.10a is **CLOSED** — localized (Plans/dns-flapping-localization-2026-08-20.md: self-inflicted via connection-semaphore coupling + wrapper-cancelled rotation + false breaker trips), fixed at the root across three merged waves (PRs #146/#147/#148), and permanently gated (hermetic contention gate + daily dns-canary.yml + the dns-probe: Lint clause; PR #149). **P2.10a itself remains deliberately un-implemented**: unbinding the 50-QPS bucket is now a pure speed/recall decision to take with post-wave data (`dns.rate_wait` in every scan-summary.json sidecar shows how binding it is) — its own decision, not part of the reliability work. Absorbed and closed with it: TF-DNS-REPRO (repro harness committed under scripts/repro/), the reddy Steps 4/7 items (semaphore decoupling shipped structurally in Wave 1; ladder timeout correctness in Waves 1-2), dnsLayer.7 (fast path timed), ISC-513's per-terminal counting half, and TF-OBSERV (every scan persists full DNS diagnostics).
>
