...
effort: E4
phase: build
progress: "ACTIVE: Task 2026-07-19c layered DNS transport resilience (ISC-514+) — network-footprint-ordered ladder DoH→DoT(853,new)→Do53(now health-gated), TransportHealth generalization + do53_health breaker (stops hammering a router-blocked port 53 = TF-DNS-FLOOD-PROTECTION), zero-new-dep tls-ring; building on branch fix/scan-log-tier-a (PR #82). Prior in this session: Tier A (whois/memory/input-guard) + #1 DNS-log + DoH-unavailable hardening shipped in #82 (fmt fix pushed); Tiers B/C/D still open"
mode: algorithm
