...
- [ ] ISC-430: PR #61 CI fully green on its head commit (probe: `gh pr checks 61` — zero fail lines).
- [ ] ISC-431: PR #61 merged to master; master CI green on the merge commit (probe: `gh pr view` state MERGED + run watch).
- [ ] ISC-432: Coverage gate re-measured on the FINAL tree (probe: llvm-cov exit 0, ≥95/95 — closes the handoff's flagged gap).
- [ ] ISC-433: Every org name entering the vendor map passes `org_role::classify_for_domain` at ONE choke point in `analysis.rs` (probe: unit test — an intermediary name from any source never lands in the map).
- [ ] ISC-434: Registry-authority SHAPE class in `classify()` — "government of", "ministry of", "agence nationale", "consulate" word-bounded patterns (probe: unit tests on the four live leaks: Tonga/.to, ANRT/.ma, Ministry of Post/.so, St. Helena/.sh).
- [ ] ISC-435: One domain → one org across the whole report: post-scan reconciliation picks the winner by tier precedence, count-weighted tiebreak (probe: quantifier on a rescan or fixture shows 0 conflicting-org domains; was 18).
- [ ] ISC-436: Pseudo-hosts and URL fragments never become vendors: `icann_suffix()==None` + "/"-containing strings rejected at discovery emit (probe: unit tests on `telemetry.eu-central-1`, `cluster.local`, the web.archive.org fragment; was 8+1 garbage vendors).
- [ ] ISC-437: Anti: no discovery-recall loss from ISC-436 — only non-registrable garbage is dropped (probe: fixture test — every ICANN-listed-suffix domain in a fixture set survives).
