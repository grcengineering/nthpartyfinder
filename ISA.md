...
- **P1.5 landed** (dedup-core, ISC-560): scan-global ScanDedup (subfinder/CT/SaaS apex memos) threaded through discover_nth_parties + process_vendor_domain like subprocessor_attempted_orgs; claim-before-run; apex-only (subdomains skip — apex always queued via add_base_domain_if_subdomain; cross-parent apex re-reach skips). Caught+fixed a critical recall bug mid-impl: gated discovery objects must be JOIN-LOCAL (*_for_join bindings), never shadow the params handed to child recursion. +1 async unit test (claim semantics). fmt+clippy-D clean; full suite 4428/0.
- **Validation scan done (ISC-566 partial):** depth-3 vanta.com on Wave-A+P1.5 binary — new counters fire live: subproc.dead_host_skip=500, subfinder.apex_skip=26, saas.apex_skip=26, dedup.org_subproc_skip=200; ct.apex_skip=0 (CT off in default config); dedup.domain_hit/dispatch_avoided/whois.cache_hit=0 (P1.1/P1.3/P1.4 not yet done — correct). No recall collapse (147500 raw -> 21397 unique, 18119 exported, 41MB report, 0 orphans). Wall-clock NOT comparable (machine contended by concurrent builds/tests). subfinder phase still dominant — needs base-keyed recursion (P1.1/P1.2) for the big cut.
- **CI fix (coverage gate):** PR #137 coverage job failed on a COMPILE error under --cfg coverage (not a %-shortfall): my P1.6 compute_baselines insertion stole the #[cfg(not(coverage))] gate from the real probe_with_logger (dup with the coverage stub) + the struct's HashMap field depended on a cfg-gated import. Fixed: re-gated probe_with_logger + fully-qualified std::collections::HashMap. Both --cfg coverage and normal builds clean; running the real 95/95 gate locally before re-push.
- **P1.3 + P1.9 landed** (ISC-557 partial, correctness core): processed_domains changed HashSet<String> -> HashMap<String,u32> (domain -> min crawl depth) on the MAIN path only (the DNS-only minimal/batch path keeps HashSet). Entry gate is now ONE atomic lock across a pure processed_gate_decision (Skip if claimed at <= depth / Claim if new-or-shallower-re-expansion / DepthRefused) — closes the two-lock check-then-insert race AND fixes the depth-blind first-wins defect (deep-first-crawl no longer permanently truncates a subtree reached shallower later). P1.9 checkpoint: completed_domains stays HashSet<String> (serialization unchanged, legacy checkpoints load); converted at the 2 sync points (processed.keys().collect()) and the resume seed (depthless set -> depth 0, conservatively never re-expanded). +5 unit tests on processed_gate_decision. fmt+clippy-D clean; full suite running.
- **P1.1 (dispatch dedup) status:** the depth-aware gate (P1.3) now makes duplicate reaches skip cleanly + counted (dedup.domain_hit). The remaining P1.1 optimization — a pre-dispatch base filter that avoids even SPAWNING the duplicate task (needs splitting edge-recording from recursion in process_vendor_domain) — is deferred as its own unit; the correctness win is already captured by P1.3.
- **P1.4 landed** (ISC-559, org singleflight): added org_cells: Mutex<HashMap<base, Arc<OnceCell<String>>>> to ScanDedup + resolve_org_once(base, compute) that runs the whole whois->web->NER chain + plausibility-gated normalization ONCE per base and coalesces concurrent callers; value cached is the FINAL normalized string so coalesced == fresh (byte-identical). Preserved the vendor_needed/customer_needed short-circuit (known bases never re-resolve) AND the concurrent vendor+customer join. whois.cache_hit counter fires on cache hits. +1 async unit test (compute runs once, coalesced value matches, distinct base runs own compute). fmt+clippy-D clean; full suite running.
- **P4.1 landed** (ISC-564 partial, accuracy): recompute_layers_bfs in app.rs re-stamps every edge's nth_party_layer as BFS distance-from-root over the FINAL deduped+filtered edge set, called at the single assemble_and_filter_results choke point after finalize. Makes reported layers deterministic + correct regardless of concurrent-DFS crawl order (pairs with P1.3's depth-aware gate + P4.2's min-layer merge). Unreachable parents keep their recorded layer (no silent zeroing). +4 unit tests (BFS distance, shallow-path, unreachable-keeps-layer, empty). Bundled with P1.4 for the wave gate.
- **P4.7 landed** (accuracy): subprocessor_attempted_orgs HashSet<String> -> HashMap<String,u32> (org key -> claimed depth). subprocessor_skip_decision is now depth-aware — a strictly shallower reach re-claims and RUNS, so a deep satellite (slack.design@3) can't suppress its shallower primary's (slack.com@2) subprocessor page (the tool's highest-evidence source). Depth-1 always runs. +1 test (shallower-primary-re-runs). Bundled with P1.4+P4.1 for the batch commit (all three staged, awaiting 1Password).
- **P2.2 core landed** (render efficiency, testable part): extracted the adaptive network-idle decision into shared pure fns should_exit_network_idle + network_idle_windows in web_traffic.rs, and refactored web_traffic's own wait loop to use them (zero behavior change). +1 unit test. The remaining P2.2/P2.3 work — wiring this idle-wait into subprocessor's render_html_in_browser + the trust-center capture (replacing the fixed 5s/2s sleeps) — needs the CDP in-flight-request event listeners (untestable render plumbing) + a validation scan, so it's the documented render-integration follow-on. Batched with P1.4+P4.1+P4.7 (all staged, awaiting 1Password).
- **P4.4 landed** (accuracy): prune_orphaned_edges in app.rs — after infra/marketing filtering drops X->infra edges, any surviving infra->Y edge has a parent that is no longer a rendered vendor node (orphaned subtree, phantom vendors in the count). Prunes to a fixpoint every edge whose parent is neither the root nor a surviving nth_party_domain; count rolls into infra_removed. Runs at the assemble_and_filter_results choke point before the P4.1 BFS layer recompute. Chose the backend prune over the frontend pass-through-node option (the review's rec, but a frontend change). +3 unit tests (phantom-parent, transitive orphan, connected-graph-untouched). Landed as its own follow-on commit after the c359b92 batch (P4.4 wiring was still uncommitted at that batch's commit time); full suite 4442 pass / 0 fail via real cargo binary, fmt + clippy -D clean.
- **P2.7 landed** (efficiency, warm-rescan): subprocessor negative cache. The measured depth-3 scan showed ~63% of vendors (515/~820) exhaust the full probe finding NO subprocessor page, and re-pay full price every scan (only successes were cached). Now `may_record_subprocessor_absence(yielded_any, budget_exhausted, any_transport_error)` records a proven absence ONLY on a full, clean, empty pass — budget-truncation (unprobed candidate tail) OR any transport-dead error (unreachable host = "couldn't look, not empty") suppresses the write, the same "an outage must never memoize as absence" discipline as `cached_urls_are_provably_stale`. Read side short-circuits `analyze_domain_with_full_options` to Ok(vec![]) when a fresh (`negative_subprocessor_entry_is_fresh`, TTL 10d, backwards-clock-saturating) entry exists AND no positive entry supersedes; a success clears the negative entry. Stored as a sibling `cache/{domain}.negative.json` (own tiny `NegativeCacheEntry`) so the positive-cache schema + its 16 constructors are untouched. New counter `subproc.negative_cache_hit` (perf 38->39). +3 tests (write-gate truth table, TTL boundary incl. backwards clock, fs write/read/clear/TTL/distinct-domain round-trip). Own commit after c359b92+P4.4; full suite 4445 pass / 0 fail via real cargo binary, fmt + clippy -D + coverage-cfg compile clean.
- **P2.1-prereq + P0.4 instrumentation landed** (observation-only, unblocks the render wave for its next validation scan): `render.retry_rescued` counter increments in trust_center `capture_with_retry` ONLY when the second capture rescued a subprocessor array the first missed — the roadmap's MANDATORY P2.1 gate ("prove the retry never rescues before removing it"); a scan-wide zero is the evidence that the second render is pure waste on futile SPAs. `report.dedup` + `report.export` timers (P0.4, `crate::perf::timed`) wrap `deduplicate_results` and `dispatch_export` at the app.rs choke points — the "last unturned stone" (is report-gen material at 15k+ relationships / is the O(S^2)-ish evidence-`contains` merge a cost?). +3 counters (perf 39->42). Zero control-flow risk (pure observation). Why now: the remaining backend items are all scan/user-gated (PROVED: P2.12's WHOIS limiter defaults to 2 QPS — routing 4,549 lookups through it would add ~38min pure pacing wall, so it needs an A/B scan to size, exactly as its probe says; P3.4-spawn_blocking touches file-write concurrency; P4.6 touches the frontend; P4.8 is recall-A/B-gated), so the correct autonomous move is the roadmap's own instrument-first philosophy + a depth-3 validation scan on the landed fixes.
- **Batch 4d5fe54 MERGED to master (ca133e3), all 8 push-CI workflows green** — P4.4+P2.7+P2.1-prereq+P0.4. Validated on a fresh depth-3 vanta.com scan (22,482 relationships, 66MB HTML): P4.4 invariant `customers-vendors-root=∅` HOLDS (0 orphans on real data); P4.1 BFS layers clean (L1 125/L2 4016/L3 18341); P2.7 lifecycle proven (2 cold writes + 1 warm hit); P2.1 `render.retry_rescued=0` across 9h (removal evidence gathered); P0.4 report-gen ~0.4s at 300k edges (immaterial, stone closed); dead_host_skip 800 / org_subproc_skip 318 / apex_skip 87 all firing; 0 orphaned processes at teardown.
- **P4.8 landed (accuracy/efficiency, recall-sensitive)** on branch perf/p48-fanout-gate: `should_gate_infra_enumeration(current_depth, base_domain) = current_depth>=2 && is_common_denominator(base_domain)` gates subdomain(subfinder)/CT enumeration for common-denominator infra/CMS roots at depth>=2 — the parent→infra edge is already recorded so the infra root stays a leaf (relationship recall preserved), only its subdomain fan-out is skipped. Wired into the subfinder_for_join/ct_for_join bindings (short-circuits BEFORE the apex claim so a gated apex isn't consumed); new counter `fanout.gate_skip` (perf 42->43) counted once per gated domain that had enumeration to skip, distinct from the apex-dedup skips. Never gates depth 1 (root always fully enumerated). Motivated by the 2026-08-15 scan: 279k subdomains, 9.15h wall, depth-3 dominated by infra fan-outs (adobe/twitter/aws.dev 5-10k subdomains each) for near-zero attribution value. +2 unit tests (depth+classification truth table; depth-gated-not-blanket vs should_stop_at_common_denominator). **A/B validation scan pending** (recall-sensitive): compare with-gate vs the 22,482-edge no-gate baseline — dropped relationships must be ONLY infra-root subdomains/descendants, never real nth-parties; the gate should also cut wall-clock sharply (the skipped fan-outs were the dominant cost).
- **Waves C+E landed (11 items, 7 parallel agents on file-disjoint groups + 2 by the orchestrator).** Render: **P2.1** (retry gated on transport-error/truncation instead of "no array" — justified by the measured `render.retry_rescued=0` across the 9h scan; settled DOM carried out of the capture and reused so the 2nd headless render is skipped), **P2.3** (trust-center 8s floor -> adaptive network-idle exit, hard cap pinned to the EXACT pre-change worst case so recall cannot regress; analytics beacons no longer reset stagnation). Scheduling: **P3.3** (browser permits -> `tokio::sync::Semaphore`, FIFO, `OwnedSemaphorePermit` acquired in async context before `spawn_blocking` — fixes the 512-thread blocking-pool priority inversion behind the 201.6s mean permit_wait), **P3.1** (the dead semaphores made real: `rg acquire src/analysis.rs` was literally 0 — admission permit now held around each vendor's LOCAL work and released BEFORE recursive descent, the leaf-held-only doctrine that makes parent-waiting-on-child deadlock impossible by construction), **P3.4** (checkpoint serialize+fsync moved to `spawn_blocking` with every mutex released first, and debounced by TIME (30s) instead of every-5-completions — a durability mechanism had become the depth-1 pipeline's own contention source). Accuracy/efficiency: **P4.6** (lossless edge-evidence merge at export), **P2.4** (tri-state fetch outcome + bounded per-URL memo kills duplicate homepage fetches), **P2.13** (NER fallback capped+batched, permit released between chunks, truncation COUNTED not silent). DNS/CT: **P2.11** (UDP race arm budget+health gated, per-server resolver cached, SERVFAIL -> `Unrelated` not `Rejected`), **P1.7** (negative memo for zone-authoritative failures ONLY + fast path routed through the memo), **P2.10b** (CNAME derived from the TXT response's type-5 answers — halves fast-path DNS volume), **P2.9** (CT `exclude=expired`, corroborated-only SAN cap, Retry-After cooldown, pooled client). +7 counters (perf 43->50). ~4,900 lines; compiles clean, clippy -D warnings clean.
  - **The suite caught 2 real bugs** (4560 passed / 2 failed) in the DNS agent's work — its own new tests, both asserting this repo's core contract. Root cause: the name-verdict preservation was wired into the `TransportFailed` arm but NOT the `Empty` arm, so when the ladder produced no records a DoH authoritative `DNS_NAME` (SERVFAIL) verdict was silently discarded as a clean empty — the name was never counted and never entered the negative memo, so every referencing domain re-paid the full four-provider rotation. That is exactly the silent-absence pattern the module exists to prevent. FIXED in both `fast_txt_lookup` and `fast_cname_lookup` by re-emitting the verdict on `Empty` too (an authoritative ladder answer carrying actual RECORDS still wins). Verified the environmental alternative first (`lsof`: nothing listening on 127.0.0.1:53) so the test was NOT weakened to pass.
  - **Deliberately NOT done, with evidence:** **P2.10a** (unbind the 50-QPS DNS bucket) stays owner-gated — the 2026-08-15 validation scan measured 343 DNS-failure warns + a full transport-ladder demotion, i.e. the "transient flapping" investigation the roadmap requires to close first is still OPEN. **CT pacing gates** stay unbuilt (empirically falsified here: 0/17 -> 1/51 completions, reverted as ISC-510). Memo sharding skipped with rationale. The DNS agent also CORRECTED the brief: `admit_do53_query()` already ANDs `do53_health.should_attempt()`, so calling both would double-consume the single-flight re-probe CAS.
- **Wave F landed (8 items, 4 parallel agents) — the final implementation wave.** **P2.6** per-URL 12s working-time envelope nested inside the 20s vendor budget, checked between render stages, permit-wait subtracted, crediting only this URL's own wait; an envelope-abandoned look can never memoize as absence (4th blocking input to `may_record_subprocessor_absence`). **P2.8** candidates re-ranked on measured evidence from 187 real cache files (114 `trust.{d}/subprocessors`, 38 `{d}/legal/subprocessors`, 8 `{d}/subprocessors`, 8 bare `trust.{d}`, long 1-2 hit tail) with the tally's own bias stated (a zero inside the probed prefix is NOT evidence of failure, so the tail is demoted not deleted); 16 candidates deleted for STRUCTURAL reasons — notably `privacyportal.{company}.com`, which welded `.com` onto the company label and therefore aimed live scanner traffic at an unrelated third party for any non-.com vendor (a real latent defect found by doing the ranking work); order-preserving dedupe added since the probe loop dedupes by source CATEGORY not URL, so duplicates were burning probe slots. **P2.14** versioned on-disk cross-scan org cache (30d TTL) mirroring SubprocessorCache's layout/sanitisation/version-gating/atomic-write, caching AUTHORITATIVE resolutions only (never a transport failure, throttle, or privacy placeholder; curated hits deliberately not cached). **P2.12 scoped**: WHOIS port-43 now inside the global connection permit + per-stage attribution counters, but enrolling the scan path in the existing limiter was REFUSED with arithmetic — its default is 2 QPS and the 2026-08-15 scan did 4,549 lookups = ~38min of pure added pacing; the roadmap's own probe demands an A/B sizing scan that has not been run, and the code says so at the call site. **P3.5** backpressure made real: permit-withholding with hysteresis modelled on the proven dns_governor shrink/restore, replacing sleeps that added latency with zero concurrency reduction. **P3.6** whole-domain wall-clock ceiling across ALL methods, permit-wait-subtracting, classified+counted+reported. **P2.15** batch shares the dedup/memo layer across entries while EDGES stay strictly per-root. **P1.2** recursion keyed on the registrable base WITH the named mitigation (web-traffic render stays keyed on the DISCOVERED hostname — `cloud.mongodb.com` captures a different third-party set than `mongodb.com`, so the naive collapse would have silently deleted those edges). **P0.2 CLOSED**: six `whois.attributed_*` per-tier counters. Perf table 50 -> 67. Gates: production `cargo build` (REQUIRED — the subprocessor production path is `cfg(all(not(test),not(coverage)))` so `cargo test` alone never type-checks it), clippy -D warnings, coverage-cfg, full suite 4623 passed / 0 failed / 22 binaries.
- **P1.1 deliberately NOT landed:** it shares one depth-aware claim primitive with the shipped P1.3 gate; a depth-blind claim there re-creates the subtree-truncation defect one layer up. Recorded rather than risked blind.
- **Remaining dedup-core (continuation, harder half):** P1.1 dispatch-dedup by base (local dedup of the recursion list, keep all edges), P1.2 recurse-on-base (with web-traffic-render-per-hostname mitigation), P1.3 depth-aware processed gate (HashSet->HashMap<String,u32>, atomic test-and-set, min-depth re-expansion) + P1.9 checkpoint boundary-conversion (keep completed_domains: HashSet serialization; convert at the 2 sync points; resume seeds depth=0), P1.4 org singleflight (discovered_vendors -> OnceCell-per-base). Waves C/D/E after. Each needs its own suite gate + the type change touches ~10 sites.
...
**Trigger (owner, verbatim → `principal_stated_goal` in frontmatter).** Owner installed v1.4.0 via Homebrew on a second Mac; the flow was clunky and the scan hung. Six screenshots supplied. Two asks: (1) Homebrew install pulls ALL deps by default; (2) the install flow is far more seamless — and specifically NPF should gracefully handle a missing Chrome instead of hanging on browser-dependent discovery.

**Root-cause map (each verified in-code, not guessed).**
1. **Deps are caveats, not `depends_on`.** `packaging/homebrew/nthpartyfinder.rb` lists google-chrome + subfinder in `caveats`, so `brew install` never pulls them → user had to decide/install manually, and subfinder/Chrome prompts fired mid-run.
2. **Chrome dep-check keys on explicit flags, not effective state.** `app.rs:1223` passes `args.enable_web_org`/`args.enable_web_traffic_discovery` (flags) into `check_dependencies`; a default run has these ON BY CONFIG DEFAULT (not by flag), so `check_chrome()` never runs (contrast subfinder/ONNX at `:1229-1230`, which correctly OR the config default). → missing Chrome undetected at "Checking dependencies".
3. **Browser launch has no timeout and is never skipped when Chrome is absent.** `browser_pool::launch_browser` → `Browser::new`/`default()` with `find_chrome_binary()` checking only `CHROME_PATH`+WSL (NOT the macOS/Linux system paths `dep_check::chrome_system_paths()` checks — an inconsistency). With Chrome absent the browser phases (subprocessor render, web_traffic, web_org) wedge → the 21-min hang at "Starting vendor discovery" 10%.
4. **Runtime data is CWD-relative, never bundled.** `vendor_registry::init` reads `config/vendors/` (219 files), `known_vendors::init` reads `config/known_vendors.json`, SaaS reads `config/saas_platforms.json` — all relative to CWD. A Homebrew binary has no `config/` → "No config/vendors directory / empty registry" + "Failed to load SaaS platforms: No such file or directory (os error 2)". (Data is small — 880K vendors + 28K — and committed; the NER model / companies.tsv are already embedded, so embedding is architecturally consistent and channel-agnostic.)
5. **Config first-run is a two-step.** `process_config_result`→`CreatedNew` (`app.rs:1135-1141`) creates the default config then RETURNS, telling the user to run again.
6. **`brew trust` gate.** Homebrew's untrusted-3rd-party-tap security gate — cannot be removed short of homebrew-core; documentable only.

**Constraints honored:** binary-native only (no scan wrapper — owner directive 2026-07-18); do NOT weaken supply-chain/coverage gates; keep a user-provided `config/` able to OVERRIDE embedded data; embedding must work across ALL channels (tarball/crates.io/docker/homebrew); code commit needs Justin's Secretive tap (never self-signed).

### Criteria (ISC-520+)

**Gates (all green on the final post-review code, 2026-07-21):** `cargo fmt --check` exit 0 · `cargo clippy --all-targets -- -D warnings` clean (also confirms review L4) · `cargo test --lib` EXIT=0 / 0 failures (+8 new tests pass) · `cargo deny check` advisories/bans/licenses/sources ok · coverage `scripts/coverage.sh` (nightly-2026-04-29 llvm-cov, `--fail-under-lines 95 --fail-under-functions 95`, browser_pool excluded) = **99.23% lines / 98.61% functions** (gate OK) · `brew style` DSL valid (only cosmetic Sorbet-sigil + placeholder-checksum-hoist offenses) · `cargo package --locked` verify-build exit 0 (crates.io path — build.rs + 219 config files ship, 1.2MiB) · release `embedded-ner` binary v1.5.0 run from an isolated CWD → `Loaded 218 vendors (embedded)` + 280 known vendors, exit 0, zero degradation warnings.

- [x] ISC-520 (Homebrew all-deps): formula declares `subfinder` + `whois` (both platforms) + macOS-only `on_macos { depends_on cask: "google-chrome" }`; the optional-deps caveats block replaced with a Linux-Chrome-only note. Evidence: `nthpartyfinder/packaging/homebrew/nthpartyfinder.rb` grep shows the 3 depends_on lines; `brew style` accepted the `depends_on cask:` / `on_macos` / `on_linux caveats` DSL (no complaint). `brew deps` on a loose file is refused by brew ("must be in a tap") → the live-tap dep resolution is verified at ship (ISC-521).
- [ ] ISC-521 (tap live): PENDING SHIP — the live tap `grcengineering/homebrew-grcengineering` must be updated with the new deps + real v1.5.0 checksums (via `scripts/sync-homebrew-formula.sh v1.5.0` after the release builds) and pushed signed. Needs Justin's Secretive tap. Falsifier: fetch the raw tap formula → contains the depends_on lines + real sha256s; then `brew install` pulls subfinder+whois+google-chrome.
- [x] ISC-522 (self-contained data): vendor registry + known_vendors + saas_platforms embedded via build.rs (`EMBEDDED_VENDOR_FILES`, 218 files) + `include_str!` (known_vendors.json, saas_platforms.json). Evidence: **LIVE** — the debug binary copied to `/tmp/npf_iso/bin` (outside the build tree, no exe-relative config) and run from an empty CWD logged `Loaded 218 vendors (embedded)` + `Vendor registry initialized: 218 vendors, 280 domains` + `Loaded known vendors database: 280 vendors`, exit 0, ZERO "empty registry"/"fallback mode"/"Failed to load SaaS platforms". Unit tests: `embedded_vendor_files_are_present`, `load_from_embedded_parses_every_file_into_a_populated_registry` (count == file count), `embedded_known_vendors_assemble_into_a_populated_database`, `embedded_saas_platforms_parse_into_a_nonempty_set` — all pass.
- [x] ISC-523 (config override preserved): Evidence — **LIVE**: a `/tmp/npf_iso/config/vendors/` with ONE vendor JSON made the same binary log `Loaded 1 vendors` (directory, NOT `218 (embedded)`), while known-vendors independently fell back to embedded (280) since no `config/known_vendors.json` override was provided — proving each source overrides independently. `vendor_registry::init` prefers `find_config_dir()`; `known_vendors::load` prefers `base_path.exists()`.
- [x] ISC-524 (Chrome detected up-front): `check_dependencies` now receives `flags.web_org`/`flags.web_traffic` (effective) at the app.rs call site (was `args.enable_web_*`). Evidence: unit test `default_run_checks_chrome_via_effective_flags` (default Args + default config → flags enable a browser phase → results include "Chrome/Chromium") + `dns_only_run_does_not_check_chrome` (the inverse). Both pass.
- [~] ISC-525 (graceful, never-hang): **redesigned after review C1** — no up-front path probe (which could false-negative a launchable Chrome). Every Chrome launch funnels through `browser_pool::acquire_tab` (grep-confirmed: web_org, web_traffic, subprocessor; no direct `Browser::new` elsewhere) and each caller degrades on Err (subprocessor→static HTML, web-org→HTTP-only, web-traffic→empty). `launch_browser` bounds each launch with a 45s `BROWSER_LAUNCH_TIMEOUT` on a helper thread (timed-out thread's eventual `TrackedBrowser` drops → Chrome reaped, no leak); the first timeout calls `note_chrome_unavailable` so the rest of the scan fails fast (no per-target 45s re-pay). A genuinely absent Chrome fails the launch quickly on its own; the dep-check (effective flags) warns up-front. Evidence: unit tests `chrome_unavailable_decision_truth_table` + `note_chrome_unavailable_latches_fast_fail` pass; `clippy -D` clean. **[DEFERRED-VERIFY]** a live Chrome-ABSENT scan (exit 0, degraded, bounded time, 0 orphaned Chrome) could not be run — this box HAS Chrome (`/Applications/Google Chrome.app`) and can't be forced off; follow-up = confirm on a Chrome-less machine (the user's 2nd Mac). The latch-on-actual-timeout design cannot wrongly disable a launchable Chrome; the hard-dep (ISC-520) also makes the absent case not arise on a Homebrew install.
- [x] ISC-526 (seamless first run): Evidence — **LIVE**: (a) non-interactive fresh dir → "Analysis completed" (proceeds); (b) PTY interactive answering "Y" → "✅ Created default configuration file" → "Using default settings for this run" → "Starting Nth Party Analysis for domain: example.com" → completed, in ONE invocation; old "run nthpartyfinder again" gone; `config/nthpartyfinder.toml` created. `--init` create-only path unchanged (still refuses to overwrite).
- [x] ISC-527 (no mid-scan install prompts on a Homebrew install): consequence of ISC-520 — subfinder is a hard dep so the "Subfinder Not Found" interactive menu (gated on absence) doesn't fire; Chrome is a hard dep so the graceful-degradation message doesn't fire. Verified by code-trace (prompts/messages gate on absence) + the ISC-526 PTY run with Chrome+subfinder present showed zero install prompts.
- [x] ISC-528 (docs): README gained a "Homebrew (macOS / Linux) — recommended" section (one-shot `brew tap … && brew install …`, auto-installs all deps, the one-time trust step, the Linux-Chrome note) + a Runtime-dependencies list for the non-brew paths; RELEASING.md Homebrew section documents the hard deps + embedded data + trust; CHANGELOG `[1.5.0]` entry. Evidence: the files.
- **Anti-claims:** (A1) never weaken the SSCS/coverage/clippy gates to pass; (A2) never reintroduce an external scan wrapper — every fix is binary-native; (A3) never remove the user-`config/`-overrides-embedded capability; (A4) never auto-sign/self-sign the code commit past Justin (Secretive tap is the ship gate); (A5) never break the raw-tarball/crates.io/docker channels — embedded data must be channel-agnostic; (A6) never claim the hang is fixed without a bounded-time completion (or a mechanically-proven never-launch), not just "should work".

**Build order (visibility + low-risk first):** ① Homebrew formula + tap (primary ask, independent, low-risk) → ② embed data (universal fix for empty-registry/SaaS) → ③ Chrome up-front detection + graceful degradation + launch timeout → ④ config first-run one-shot → ⑤ docs. Then in-family adversarial review (codex unauthenticated → Forge/Cato blocked; fresh-context skeptic substitutes, per prior NPF sessions) + full gates + owner tap.

**Adversarial review disposition (fresh-context skeptic, ~214k tok; Forge/Cato blocked — codex unauthenticated):**
- **C1 (CRITICAL) — ADOPTED (design change).** The Chrome graceful-degradation originally set a browser-pool latch from an up-front `check_chrome` path probe, then hard-disabled web-traffic + failed `acquire_tab` fast on it. The probe is NARROWER than the actual launcher (`check_chrome` misses `PATH` lookup, `/snap/bin`, the WSL `/mnt/c/...` mount `find_chrome_binary` supports, and `~/Applications`), so a false-negative would wrongly disable a Chrome the launcher COULD find — a real regression on Linux-snap/WSL. (Reviewer overstated it — `chrome_system_paths`/`chrome_install_hint` ARE `#[cfg]`-gated per-OS, so Linux does check `/usr/bin/chromium` and shows the apt hint; L1 is void, C1's "no Linux paths" is void — but the PATH/snap/WSL/`~/Applications` gap is real.) **Fix:** dropped the probe-driven latch + `dep_check::check_chrome_availability` + `browser_pool::set_chrome_available`/`browser_launches_enabled` + the app.rs disable/message. The latch is now set ONLY by an actual launch timeout (`note_chrome_unavailable`), never a heuristic — so Chrome present anywhere the launcher finds it always attempts and works. Missing/wedged Chrome still degrades: dep-check warns (effective flags), the 45s launch timeout bounds a wedged launch + latches so the rest fail fast, a genuinely absent Chrome fails the launch quickly on its own, and every browser caller degrades (subprocessor→static, web-org→HTTP, web-traffic→empty via the coverage manifest). This also resolves **L2** (per-target timeout: the first timeout now latches).
- **M1 (MEDIUM) — MITIGATED, kept.** A hard `depends_on cask: "google-chrome"` can error if an unmanaged Chrome already sits in `/Applications`. Kept the hard cask dep (honors the explicit "install ALL dependencies" ask + the graceful degradation backstops it) and added a caveat: `brew install --cask --adopt google-chrome`. This box HAS an unmanaged `/Applications/Google Chrome.app` → the ship-time `brew install` from the tap empirically exercises exactly this case; if it errors, switch to the adopt path or a soft recommend.
- **M2 (MEDIUM) — REBUTTED.** Reviewer claimed `brew trust` isn't a real command (reasoning from real-world Homebrew 4.x). In THIS environment `brew trust --help` returns "Trust non-official tap formulae…" (exit 0) and the user's own screenshots show `brew trust grcengineering/grcengineering` → "Trusted tap: grcengineering/grcengineering". The docs are correct.
- **L1 — VOID** (`chrome_install_hint` is `#[cfg]`-gated; Linux gets the apt hint). **L3 — ACKNOWLEDGED** (build.rs panics if `config/vendors` unreadable — appropriate: the files are git-tracked under the crate root; a clear build error, not a runtime one; noted). **L4 — VERIFIED CLEAN** (`cargo clippy --all-targets -- -D warnings` passes; the `?`-closure doesn't trip `redundant_closure_call`).
- **Sound areas confirmed by the review:** embedded-data override semantics (all 3 modules), build.rs correctness/determinism/`cargo package` compatibility, latch drop/Send/no-leak, `flags` hoist behavior-preservation, config first-run.

---

## Task 2026-07-19c — Layered DNS transport resilience (DoT tier + per-transport circuit breakers)

**Trigger (owner, verbatim).** After a design discussion on DNS transport fallback ordering ("To ensure that Nth Party Finder's DNS lookups 'can't fail', how else should we layer other protocols… 1.) DoH3; 2.) DoH; 3.) DoQ; 4.) DNS over TCP; 5.) DNS over UDP … keep me honest, I might not be thinking about this … optimally balances speed, reliability, host system stability, and network stability"), owner ran `/goal Add it to the ISA for tracking purposes and then implement --> test --> commit --> push`.

**Design reframe (kept honest — the answer that drove the build).** The naive "more transports, speed-first" ladder degrades TOWARD the flood-prone transports (raw UDP/53 at the *bottom* = the conntrack-exhaustion vector behind the recurring WiFi collapse, TF-DNS-FLOOD-PROTECTION). Correct ordering is **network-footprint-FIRST**: connection-pooled/multiplexed encrypted transports (DoH, DoT — a handful of long-lived TLS conns carry thousands of queries) BEFORE raw Do53 (thousands of ephemeral UDP flows lingering in NAT). Three corrections to the premise: (1) transport diversity ≠ reliability — resolver/provider diversity is the real axis (already present via rotation + GRC-367); (2) "can't fail" is the wrong target — authoritative NXDOMAIN/NODATA are *correct answers*, and retrying them across transports is the amplifier, not resilience; (3) hedge + failure-class-gating + per-domain budget beat a deep serial ladder (serial worst-case latency = SUM of timeouts; that was the 3s-per-lookup DoH bug ×5).

**What NPF already had (verified in-code).** DoH (443, health-gated by `DohHealth`) → direct Do53/UDP (rotating public resolver, **UNGATED**) → system resolver; plus failure-class-aware caching (an authoritative empty is a cacheable fact; an all-paths-failed empty is not — dns.rs:170-172/1413/1452). The gaps: (a) the Do53 fallback had **no circuit breaker** — on a router rate-limiting/blocking port 53, every lookup kept hammering the exact port-53 path that triggers the flood protection (self-amplifying outage); (b) no encrypted transport between DoH and raw Do53, so a DoH-blocked network degraded *straight to* the flood-prone UDP path.

**Implemented (this task).**
1. Generalized `DohHealth` → `TransportHealth` (byte-identical circuit-breaker logic: `TRANSPORT_DOWN_THRESHOLD`=8 consecutive failures → skip the transport + a single-flight re-probe every `TRANSPORT_REPROBE_INTERVAL_MS`=30s; any success resets the streak). Now reused for all three transports.
2. NEW **DoT (DNS-over-TLS, 853)** tier inserted between DoH and Do53 — encrypted, TCP-pooled, low conntrack footprint. Built via hickory `NameServerConfig::tls(ip, server_name)` (feature `tls-ring`, **ZERO new crates** — reuses the rustls/ring/webpki stack `https-ring` already pulls in). One resolver holding Cloudflare+Quad9+Google DoT endpoints (resolver-level provider diversity). Health-gated by `dot_health`.
3. NEW **`do53_health` circuit breaker** on the raw-UDP/53 tier — the safety win: on a port-53-blocking/rate-limiting router NPF stops firing Do53 after 8 consecutive TIMEOUTS (backs off, single-flight re-probe every 30s) instead of amplifying the outage → directly addresses TF-DNS-FLOOD-PROTECTION.
4. Failure-class-correct `DirectOutcome{Answered,Empty,TransportFailed}`: the shared breakers count **only** transport failures (outer `tokio::time::timeout` elapsed = no response = the flood-block symptom), never authoritative empties (which are the NORM for CNAME) — else the shared breaker would false-trip on empty-CNAME scanning and wrongly disable Do53 for TXT too. A defensive, unit-tested string classifier (`resolver_error_is_transport_failure`) is the tiebreaker for the rare in-budget resolver error; unknown errors default to Empty (conservative — never false-trips).

**Deferred (documented, not built — surfaced to owner per rule 10).** DoQ (853, `quic-ring`→quinn/QUIC stack) and DoH3 (reqwest http3 feature) — bigger dependency + supply-chain surface, less battle-tested, lower marginal value once DoT gives QUIC-free encrypted+pooled fallback; **TF-DOQ / TF-DOH3**. A persistent pooled DoT resolver to amortize the TLS handshake across a long outage — **TF-DOT-POOL** (per-call build mirrors the existing Do53 pattern; the DoT tier only runs during a DoH outage, so handshake cost is bounded by `dot_health` tripping). Staggered hedged requests + a global per-domain resolve budget — **TF-HEDGE** / ties to ISC-439/TF-CONN-CEILING.

**Adversarial review (in-family fresh-context skeptic; codex installed but UNAUTHENTICATED this session → Forge/Cato couldn't run — noted, not substituted).** Verified against the vendored hickory 0.26.1 source. Found + FIXED 2 real defects + 1 stale comment; 5 attack surfaces confirmed SOUND.

- **[HIGH] FIXED — the Display-string classifier false-tripped the breaker on authoritative NODATA.** hickory 0.26 returns NODATA/NXDOMAIN as `Err(NoRecordsFound)` (NOT `Ok(empty)`), and its `Display` embeds the queried NAME — so a lookup for `mtls.example.com` / `connection.foo.com` rendered a string containing "tls"/"connection", read as TransportFailed → false-trip on an authoritative empty during subdomain fan-out (the exact silent-false-negative `DirectOutcome` exists to prevent). Fix: replaced the string classifier with a TYPED one — `net_error_is_authoritative_negative` = `matches!(e, NetError::Dns(_))` → Empty (the server spoke DNS = transport works); every other `NetError` variant → TransportFailed. Domain-name-independent. New unit test constructs `NetError::Dns(DnsError::ResponseCode(NXDomain))` — the exact case the string version got wrong.
- **[MEDIUM] FIXED — per-lookup DoT resolver rebuild defeated connection pooling.** `build_dot_resolver()` ran on EVERY DoT lookup → a fresh TLS handshake per lookup → up to 128 concurrent short-lived :853 connections during an outage = the connection-churn the ladder exists to prevent. Fix: cache the DoT resolver in a per-pool `tokio::sync::OnceCell<Option<TokioResolver>>` (built once on first DoT use, reused → hickory pools its :853 connections).
- **[LOW] FIXED — stale "inner-timeout is the sole authority" comment.** Typed classification makes both the inner (`NetError::Timeout`) and outer (tokio `Elapsed`) timeouts resolve to TransportFailed regardless of which fires; comment corrected.
- **SOUND (verified against code):** the empty→None throttle-surface contract (exit-3 guard unaffected — throttles counted at `note_throttle` independent of the ladder return), DoT-`Empty` falls through to Do53 (no recall loss; only a DoT `Answered` short-circuits), `next_dns_server_opt`+`create_dns_resolver` shared-borrow soundness, `should_attempt`'s single-flight CAS + `record_failure` warn-once, coverage/`cfg` gating, and no panic/unwrap path in `build_dot_resolver` (`.ok()`/Option throughout).

### Criteria (ISC-514+)

- [x] ISC-514: `TransportHealth` generalizes the DoH breaker with identical semantics (8-failure trip, 30s single-flight re-probe, success-resets) and is instantiated per-transport (`doh_health`/`dot_health`/`do53_health`). Evidence: `cargo test --lib dns::` = **271 passed / 0 failed** incl. the 4 renamed `doh_health_*`/`TransportHealth` gate tests; struct now carries all 3 instances (dns.rs field block + 3 constructors).
- [x] ISC-515: a DoT resolver builds over the well-known 853 endpoints and actually resolves. Evidence: `build_dot_resolver_is_constructible` (no I/O at build) passes; the live `#[ignore]` `dot_resolver_resolves_a_known_txt_live` **PASSED** locally against 1.1.1.1/9.9.9.9/8.8.8.8:853 (`test result: ok. 1 passed`), returning ≥1 TXT record for cloudflare.com over real DoT. (Needed the `webpki-roots` feature for trust anchors — first run returned empty because `tls-ring` alone ships no roots; fixed.)
- [x] ISC-516: the `do53_health` breaker trips on sustained transport failure but NEVER on authoritative empties. Evidence: `resolver_error_classifier_separates_transport_failures_from_negatives` (asserts "no record found…"/NXDOMAIN→Empty; "request timed out"/"connection refused"/"no connections"/os-error/TLS→TransportFailed; unknown→Empty) + `transport_health_empty_answers_never_trip_the_breaker` (3×threshold consecutive `record_success` keeps `should_attempt` true) both pass. `DirectOutcome` keys TransportFailed on the outer-timeout (the flood-block symptom), never on an authoritative empty.
- [x] ISC-517: the ladder is DoH→DoT→Do53, each tier skipped when its breaker (or `dot_enabled`) is down, first NON-EMPTY authoritative result wins; an authoritative empty resets the breaker but doesn't mask a DoH throttle. Evidence: code read of `fast_txt_lookup`/`fast_cname_lookup`/`laddered_direct`; the 3 GRC-367 throttle-surface tests still pass (proving the empty→None contract held).
- [x] ISC-518: ZERO new dependencies — `tls-ring` + `webpki-roots` both reuse crates already in the tree. Evidence: `cargo tree` distinct-crate count **362 → 362** (measured with both features on; `webpki-roots` v1.0.7 already pulled by reqwest); `cargo deny check` = "advisories ok, bans ok, licenses ok, sources ok" (exit 0).
- [x] ISC-519: the healthy path is unchanged — when DoH answers, DoT/Do53 are never reached; two relaxed atomics per lookup on the hot path. Evidence: live `vanta.com -r 1 --dns-only` scan **found 15 vendor relationships, exit 0, ZERO** "DoT"/"Direct DNS (UDP/53)"/"blocked or unavailable" warnings in scan.log (grep count 0).
- [~] ISC-520: full gate green on the branch — fmt ✓, `clippy --all-targets --all-features -D warnings` ✓, full `cargo test --lib` = **4277 passed / 0 failed** (exit 0) ✓, `cargo deny check` ✓. Coverage ≥95/95: **PENDING** (cargo-llvm-cov run in progress; new network fns are `#[cfg(not(coverage))]`-excluded like the old fallbacks, and the pure helpers are unit-tested, so ≥95/95 expected).
- [x] ISC-521 (healthy-path portion): live monitored scans (grc.engineering + vanta.com, depth-1 dns-only) resolved via the DoH path with clean exit, **zero orphaned** nthpartyfinder/subfinder (`pgrep`=0), `headless_shell`=0, and Mac DNS OK before AND after (`dig` exit 0). The DoT transport is independently proven by the live 853 probe (ISC-515). **[DEFERRED-VERIFY]** the DoH-*blocked* ladder end-to-end (DoT actually carrying the scan) — needs a sudo firewall rule blocking 443, owner-present; folds into TF-DNS-REPRO. Follow-up task: the DoH-blocked live validation.

**Anti-claims (MUST NOT).** Increase steady-state UDP/53 traffic on a healthy network (DoH still answers first, so the new tiers are outage-only); introduce any new dependency; false-trip a breaker on authoritative empties (esp. empty CNAME); alter the DoH throttle-counting semantics that feed the exit-3 guard (throttles are counted at `note_throttle` before this code, unchanged); regress any existing `dns::` test.

## Task 2026-07-19b — DNS-outage RCA on the owner's UDM + DoH-unavailable hardening

**Trigger (owner).** During the scan-log work, the owner's Mac lost DNS mid-session; owner then asked me to (1) access his UniFi Dream Machine directly to "pinpoint precisely what the problem is + remediate & avoid it going forward", then (2) "implement your proposed fix; however keep direct DNS as a fallback for networks that block DoH; consider other DoH-unavailable scenarios and compensate." Owner chose "reproduce, then harden."

**RCA (direct UDM SSH, read-only) — CORRECTED my earlier hypothesis.** I had claimed NPF flooded the UDM's "DNS Shield" (dnscrypt-proxy). Direct evidence **disproves** it: (a) the UDM's dnscrypt query log flowed **continuously through the outage** (09:56=38/min … no gap) — the UDM served other clients fine; (b) the scan-window dnscrypt rate was **~55-106/min vs a ~53/min quiet baseline** — NOT a flood — and the domains were mostly `unifi.localdomain`/`google.com`, because **NPF resolves over DoH (443), bypassing the UDM's :53 entirely**; (c) NPF's code is already DoH-first (3s DoH → single rate-limited+connection-capped UDP fallback) and subfinder runs passive; (d) the UDM logged **no** block/rate-limit/threat action against the Mac (192.168.1.196 absent from today's messages), Suricata/IPS not running, conntrack 976/65536 (not full), 0 OOM. **Conclusion: the UDM did not fail and NPF did not flood; the port-53 loss was Mac-specific + transient, cause not definitively isolable from retained logs.** (UDM detail: transparent :53 REDIRECT→:20201 dnsmasq→dnscrypt-proxy→Quad9/Cloudflare; conntrack UDP timeouts 30s/180s; dnscrypt `cache=false`.)

**Reproduction — PENDING (blocked on 1Password).** Built a dual-side harness (`scratchpad/dns-mon.sh` Mac DNS-health monitor + kill-switch; UDM `tcpdump` of the Mac's :53) to catch the real cause under a controlled scan. Blocked: `op read`/`op run --env-file` both fail (1Password CLI **authorization timeout**; the env-file FIFO has no writer). Awaiting owner 1Password re-auth. TF-DNS-REPRO.

**Hardening — IMPLEMENTED (this is the "compensate for DoH-unavailable" the owner asked for).** `dns.rs`: added `DohHealth` on `DnsServerPool` (shared TXT+CNAME). After 8 consecutive DoH failures, mark DoH down and **skip the 3s DoH round-trip**, going straight to the existing direct-DNS fallback (still `dns_limiter`-rate-limited + connection-capped); re-probe DoH every 30s (single-flight CAS) to auto-resume; warn once per outage. Healthy path unchanged (2 relaxed atomics/lookup); any DoH success resets the streak (flaky-but-working DoH keeps being used). Evidence: 4 `dns::tests::doh_health_*` unit tests + full `dns::` module 267 pass; fmt+clippy(-D) clean. Branch `fix/doh-unavailable-fallback`. **Note:** since NPF wasn't the proven cause, this is defense-in-depth for DoH-blocked networks, not a flood fix. No bulk-DoH endpoint exists (DNS is one-question-per-query; ANY is dead per RFC 8482) — HTTP/2 multiplexing (already via reqwest) + `answer_memo` cache are the round-trip levers, and reducing query count isn't needed anyway.
...
**⚠️ Live finding — TF-DNS-FLOOD-PROTECTION (likely the mechanism behind the recurring "WiFi collapse").** After several back-to-back depth-2 vanta.com validation scans, the machine's DNS went down. Precise diagnosis (not generic conntrack): **port 53 SPECIFICALLY is blocked — BOTH UDP and TCP — while everything else is perfectly healthy.** Evidence: `ping 192.168.1.1` and `ping 1.1.1.1` both 0% loss; `nc -z 140.82.112.3 443` (GitHub) and `nc -z 1.1.1.1 443` both succeed; but `dig` AND `dig +tcp` to 1.1.1.1 / 8.8.8.8 / the router all time out; `git push` fails "Could not resolve host". Port-53-both-transports-blocked-while-443-fine = the **router's DNS-flood protection / port-53 rate-limiting** triggered by the scans' DNS query volume (NPF's own lookups use DoH/TCP-443, but **subfinder + any native-resolver path emit UDP/53**), with a cooldown >10 min. **This very likely IS the "WiFi collapse" the owner keeps hitting:** DNS blocked → every DNS-dependent app fails → looks like the whole network died (though TCP/ICMP still work), and "switching SSIDs recovers it" fits — a different band/interface has separate DNS-protection state. Clears on router cooldown or a router/SSID reset. **Lessons:** (1) even the PR#80/#81-hardened binary degrades DNS when run repeatedly/deeply — space validation out, prefer shallow/fewer, one scan at a time; (2) real fix worth building — a UDP-DNS query-rate + inflight ceiling analogous to the TCP connection ceiling, or route subfinder's resolution through the same DoH/TCP-443 path so NPF stops emitting a UDP/53 flood at all. Halted all scans on detection; no leftover NPF processes.

**Tier B/C/D — open (not yet built):**

- [~] ISC-510 (#2 CT) — **DEFERRED after live validation disproved the plan's premise; gate reverted → TF-CT-DECOUPLE.** Implemented a per-provider `ProviderGate` (rate + concurrency) + CT keep-alive override and validated it live on depth-2 vanta.com. Findings: (1) baseline confirmed the problem — 92/98 CT queries empty = 6.1% success; (2) a tight 2-concurrent gate made it **worse** — CT `discover()` is awaited **inline in each vendor's processing budget**, so serialized queries queued past that budget and **0/17 completed**; (3) retuned to 4 rps + 16-concurrent (sustained-rate is the real throttle trigger, not concurrency) — the scan got safer (**peak sockets 68 vs 670**, 253 rels, 0 network issues) but CT recall did **not** improve: **1 of 51 CT queries completed**. Root cause: crt.sh goes *silent* under load (queries hang to the 30 s timeout holding gate permits rather than 429-ing fast), and inline-per-vendor CT can't finish within the vendor window regardless of gate tuning. **The real fix is architectural — decouple CT discovery from the per-vendor critical path (its own phase/budget, or a shorter fail-fast CT timeout + background aggregation), not a pacing knob. Reverted the gate (unshippable: doesn't meet the recall goal, network benefit overlaps PR#80's ceiling and isn't cleanly isolable).** TF-CT-DECOUPLE captures the redesign.
- [x] ISC-513 (#1 DNS honesty) — **scoped down after empirical check: the fix is VISIBILITY, not de-amplification.** The RCA feared ~5–10× per-attempt amplification, but a depth-2 vanta.com run showed the reported count (116) ≈ distinct terminal failures (124 lines / 104 distinct domains) — NOT meaningfully amplified. The severe depth-3 number (47,978) was a symptom of the EMFILE/conntrack collapse **PR #80 already fixed** (EMFILE counts once + stops rotating). So the counting is left untouched (exit-3 guard intact). The real gap: the whole summary block — DNS-failure count + SUCCESS/WARNING verdict — is `println!` to stdout, so scan.log ended at "Export completed" with **zero** DNS-failure info (verified). Fix: `print_final_summary` mirrors the count + verdict into the `--log-file` sink via a file-only helper (`log_summary_to_file`; `export_logs` runs after it in app.rs). Evidence: `logger::tests::dns_failure_summary_is_mirrored_to_log_file` + `cargo test --lib logger::` 113/0; **live depth-1 vanta.com run confirmed scan.log now carries `INFO: Analysis summary — …` + `WARN: DNS resolution failures: 3 (some vendors may be missing)`, matching the console (3=3), where before it had zero DNS-failure info.** De-amplification + `log_dns_lookup_failed` wiring were NOT needed (would flood scan.log with per-domain lines) → not done by design.
...
**Follow-up: retire the wrapper entirely (owner directive, 2026-07-18).** After #80 merged, owner: "I never want to use a safe scan wrapper script when using nthpartyfinder going forward. Any safety-related mechanisms must always be baked directly into the binary itself. The safe scan wrapper was a temporary measure." Audited `scripts/safe-scan.sh` against everything now in the binary: (1) `ulimit -n 512` — superseded, the binary self-raises to 8192; (2) `--parallel-jobs`/`--dns-rate-limit`/`--http-rate-limit`/`--whois-concurrency`/`--max-retries` caps — superseded, the connection ceiling (#68) + #80's browser/subfinder caps bound sockets directly without throttling rate (exactly the design goal the original Round-2 plan named for relaxing these once validated); (3) post-sweep on exit — superseded for every path the binary can itself intercept (`shutdown()`/`PoolShutdownGuard`/panic hook/Ctrl-C, the last extended to subfinder in #80). **The one genuine gap:** the wrapper's PRE-scan sweep of Chrome orphaned by a *previous* hard-killed run (`kill -9`, or a crash before any handler could fire) — none of the binary's own reap paths can see a predecessor's leftovers, since they only track PIDs *that* run itself launched. Closed by adding `browser_pool::sweep_orphaned_chrome()` + `discovery::subfinder::sweep_orphaned_subfinder()` (both `cfg(unix)`, system-wide `sysinfo` process-table scan for `ppid==1`/no-live-parent processes matching our Chrome profile-dir signature `rust-headless-chrome-profile` or the subfinder name, called at `run_inner` startup before any new Chrome/subfinder spawns) — mirroring the wrapper's exact discriminators (signature match, not name-only, for Chrome, so the operator's own real browser or another tool's headless Chrome can never be swept). Removed `scripts/safe-scan.sh`; reworded 5 doc comments that referenced it (Cargo.toml, http_client.rs, app.rs, browser_pool.rs ×2) to describe the binary-native mechanism instead; added the standing "no wrapper scripts" rule to project `CLAUDE.md`. Branch `fix/binary-native-orphan-sweep` off `origin/master` @ `2f23c1d` (post the 9-dependabot-merge tip). New unit tests: `test_is_orphaned_chrome_of_ours_requires_signature_and_no_parent`, `test_is_orphaned_subfinder_of_ours_requires_name_and_no_parent` (pure predicates; the live sysinfo scan itself is `coverage(off)`, same convention as the existing PID-registry reap functions). Temp profile dirs are NOT cleaned by the sweep (a `kill -9` runs no destructors in the killed process either, so its dir is left regardless of which reap path fires) — accepted disk-hygiene tradeoff, unchanged from the wrapper's own behavior (it never cleaned dirs either, only killed processes).
...
- [x] ISC-435: One domain → one org across the whole report: post-scan reconciliation picks the winner by tier precedence, count-weighted tiebreak (probe: quantifier on a rescan or fixture shows 0 conflicting-org domains; was 18).
- [x] ISC-436: Pseudo-hosts and URL fragments never become vendors: `icann_suffix()==None` + "/"-containing strings rejected at discovery emit (probe: unit tests on `telemetry.eu-central-1`, `cluster.local`, the web.archive.org fragment; was 8+1 garbage vendors).
- [x] ISC-437: Anti: no discovery-recall loss from ISC-436 — only non-registrable garbage is dropped (probe: fixture test — every ICANN-listed-suffix domain in a fixture set survives).
- [ ] ISC-438: Anti: increment-2 changes keep all repo gates green (fmt, clippy --all-targets, full suite, coverage ≥95/95, cargo deny).
- [ ] ISC-439: Subprocessor analysis carries a GLOBAL per-domain time budget — a uniformly slow origin cannot stall a scan (probe: hermetic slow-TLS-origin fixture completes < budget). **Evidence this is a live gap: httpbin.org's 2026-07-12 degradation made `analyze_domain` take 93s wall-clock (only per-request timeouts exist; sequential URL probes accumulate).** [INCREMENT 2]
...
- **2026-07-12 (increment 2 scoping — quantified, not estimated):** Ran a deterministic defect census on the preserved AFTER scan (`~/Documents/npf-attribution-evidence/scan-AFTER-branch.json`) instead of trusting the handoff's estimates. Findings that re-ranked the plan: (1) conflicting-org domains = **18**, not 55 — and in most, the correct answer is already present on one edge (mcsv.net "Intuit" vs "Mailchimp", siteforce.com "Salesforce" vs echo) so reconciliation is a precedence pick, not new resolution; (2) `cursor.sh` ships "Government of St. Helena" on one edge and "Cursor" on another → **at least one emit path bypasses org_role entirely** (the gate is consulted only at whois.rs:488/:935) → root-cause fix is ONE choke-point gate at the vendor-map insert in analysis.rs, which also immunizes against stale cached WHOIS replaying pre-gate data; (3) four ccTLD registry-authority leaks are LIVE (Tonga/.to, ANRT/.ma, Ministry of Post/.so, St. Helena/.sh) — the name-by-name denylist can't scale; add a SHAPE class (government/ministry/agence/consulate) beside the existing redaction-shape precedent; (4) **known_vendors expansion DEMOTED**: the top of the 1,304-domain "echo" list is github.com(328)/atlassian.com/apple.com — already-CORRECT answers that equal the domain label after legal-suffix normalization; expanding the curated tier there renames right answers; real coverage gains live in the long tail and need W4 provenance to even measure; (5) one web.archive.org URL fragment shipped as a vendor domain → discovery-side parse guard joins TF-PSEUDODOMAIN (8 pseudo-hosts confirmed). refined: increment 2 = ship PR #61 → uniform gating (ISC-433) + registry-authority shape (ISC-434) + reconciliation (ISC-435) + pseudo-host/URL guard (ISC-436/437).

- **2026-07-12 (TF-FLAKY-HTTPBIN filed):** `test_analysis_timeout_handling` (tests/subprocessor_integration_tests.rs:52) calls LIVE `httpbin.org` and asserts wall-clock < 60s — it failed CI on `c26d66d` purely on third-party latency (same job green on `93cf40b` 40 min earlier; the intervening delta is string-matching in org_role + a Dockerfile COPY + docs, none on that path). Remediated by job rerun this time. Real fix (increment 2): point the test at a local wiremock with a delayed responder so the timeout path is exercised hermetically — a live-network wall-clock assert in CI taxes every future PR with coin-flip reruns.
...
### Increment 2 pre-merge hardening (2026-07-12, commits 93cf40b/c26d66d/9e17a1c)
- ISC-430: PASS — `gh pr checks 61 --watch` exit 0 on head `9e17a1c`, 27/27 pass, 0 fail. Master `23be09c` verified still an ancestor (CI tested the exact tree that merges).
- ISC-432: PASS — coverage gate re-run on final tree: exit 0 at 99.3% lines / 98.7% functions (closes the handoff's "never measured on final tree" gap).
...
| Metric (7,531 real relationships) | Before (PR #61 on master) | After increment 2 |
|---|---|---|
| Conflicting-org domains | **18** | **0** |
| Pseudo-host vendors | **11** | **0** |
| Intermediary org names | **13** | **2** (both are honest fallback labels that trip a shape rule — `gdpr.eu` → "Gdpr"; output already correct) |

- ISC-433: PASS — `gate_intermediary_orgs` re-checks EVERY org in EVERY position (vendor, customer, root) against `classify_for_domain`, so no emit path can bypass the gate. 45 names gated on the real scan. This is why `cursor.sh` shipped as "Government of St. Helena": the gate was only called at whois.rs:488/:935, so a warm cache entry or a later-added path skipped it silently with no test failing. A gate remembered per-call-site is a convention; conventions rot.
- ISC-435: PASS — `reconcile_org_per_domain`: one domain → one org report-wide. Precedence: offline curated authority (`resolve_curated`) > most-supported real name > honest label. Deterministic under ties (total sort order; 5x repeat test). 17 domains reconciled; conflicts 18 → 0.
- ISC-436: PASS — `is_non_registrable_host`. **The handoff doc's proposed fix was WRONG and would have caused silent recall loss:** it said to wire `icann_suffix() == None` into the emit path, but that returns None for `github.io`, `s3.amazonaws.com`, `vercel.app`, `pages.dev`, `web.app` — all real vendors in the PSL's PRIVATE section. Empirically probed before writing a line. The correct discriminator is whether the PSL lists the suffix in EITHER section: a pseudo-host's suffix is in NEITHER, so `psl::suffix().typ() == None` (the implicit-`*` rule). 12 rows dropped (incl. the archive URL fragment and `anysphere, inc.` — an ORG NAME that had leaked into the domain field).
- ISC-437: PASS(Anti) — recall proven on real data: of 1,647 domains, exactly 11 removed and **every one is provably non-registrable** (harness asserts `bad_losses.is_empty()`). Unit test `finalize_never_drops_a_real_relationship` pins the private-section platforms.
- ISC-438: PASS(Anti) — fmt 0, clippy `--all-targets -D warnings` 0 (real exit codes, not read through a pipe — a `| tail -1` masked 2 clippy errors once this session, the exact hazard the global rule names), lib suite **4,261 pass / 0 fail**, coverage `finalize.rs` 98.74% / `org_role.rs` 99.48% / TOTAL 99.32 lines · 98.68 fn, cargo deny ok.

**Adversarial review (in-family, 26 agents: 4 lenses → per-finding refutation; TF-CATO still open — codex AND MOONSHOT_API_KEY both absent, so NO cross-vendor pass is claimed). It caught a CRITICAL regression I had introduced, and one the codebase had already made and reverted once:**
- **C1 — the uniform gate destroyed curated OWNERSHIP assertions.** `org_role`'s denylist names real companies (`cloudflare`, `godaddy`, `neustar`, `markmonitor`) because they are registrars — but registrars *own domains*. The crate's own dataset says `cloudflareinsights.com`/`cloudflarestream.com`/`videodelivery.net` → "Cloudflare, Inc.", `trafficfacts.com` → "GoDaddy", `agkn.com` → "Neustar". The self-attribution rescue can't save them (not eponymous), so my gate rewrote them to "Cloudflareinsights", "Trafficfacts", "Agkn" — **five fake orgs in place of three real ones, the exact incoherence the module exists to remove.** `whois.rs:192-196` documents this precise mistake being made and reverted before ("registrars really do own domains… Filtering the dataset through it threw away correct answers"). Fixed with `is_curated_assertion`: a curated row is a *positive ownership claim*, never a registrant-field *leak*, so the filter must not touch it. `canonical_org` had the same guard and so could not heal it — the safety net was disabled too; guard removed.
- **C2 — the state-authority shape (shipped in PR #61) disowns real companies.** `classify_for_domain` ran the shape rule BEFORE the self-attribution rescue, so **"Ministry of Supply"** (a clothing company) and **"Ministry of Testing"** were classified `RegistryOperator` on their own domains. Rescue now runs first (redaction check still precedes it, since redactions QUOTE the domain they hide). `is_self_attribution` also had to learn whole-name matching: its `len() >= 3` word filter dropped "of", so "Ministry of Supply" never matched `ministryofsupply`.
- Also fixed: `reject_non_registrable` was asymmetric (vendor position only → a pseudo-host in the customer position kept a name and a vote); a single implausible scraped tagline could outvote N honest labels (winner now must pass `is_plausible_org_name` + the role gate); `FinalizeStats` under-reported (a *unanimous* report superseded by the curated tier rewrote every row while reporting "0 reconciled" — now counts rows actually rewritten: 141, not 17).
- Every finding is pinned by a regression test (`a_registrar_keeps_the_domains_it_actually_owns`, `a_company_named_like_a_state_still_owns_its_own_domain`, `one_scraped_tagline_does_not_outvote_the_honest_label`, `a_pseudo_host_in_the_customer_position_is_dropped_too`, `a_unanimous_rewrite_is_counted_honestly`).
- **Measuring-instrument bug too:** my own census predicate classified curated rows as intermediaries — the same bug as C1 — and would have reported correct answers as defects. Fixed before quoting any number.
- Idempotence: `finalize_is_idempotent` — a resumed scan re-finalizes; a second pass must change nothing or the report drifts.
- Counts are surfaced via `logger.info` (dropped / gated / reconciled), never silent: a stale PSL snapshot or an over-eager gate shows itself in the run output.
